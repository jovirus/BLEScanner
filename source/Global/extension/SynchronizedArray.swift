//
//  SynchronizeArray.swift
//  nRF Connect
//
//  Created by Jiajun Qiu on 20/09/2017.
...
public class SynchronizedArray<Element> {
    ...
    public typealias executive = (([Element]) -> Void)
    
    init(debug: String) {
        self.queue = DispatchQueue(label: "com.nordicsemi.nrfconnect.SynchronizedArray" + debug, attributes: .concurrent)
        self.array = [Element]()
    }
}
...
public extension SynchronizedArray {
    ...
    func remove(at index: Int, completion: executive? = nil) {
        queue.async(flags: .barrier) {
    ...
    ///   - completion: The handler with the removed element.
    func remove(where predicate: @escaping (Element) -> Bool, completion: executive? = nil) {
        queue.async(flags: .barrier) {
    ...
    func removeAll(completion: executive? = nil) {
        queue.async(flags: .barrier) {
    ...
    ///
    /// - Parameter completion: The handler with the removed elements.
    func removeFirst(completion: executive? = nil) {
        queue.async(flags: .barrier) {
